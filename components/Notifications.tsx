  ...
  return (
    <div className="max-w-2xl mx-auto pb-20">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <p className="text-gray-500 mt-1">Messages importants du comité de pilotage.</p>
      </div>

      <div className="space-y-4 mb-10">
        {notifications.length > 0 ? (
          notifications.map(notif => (
            <div key={notif.id} className="bg-white p-6 rounded-xl shadow-sm border-l-4 border-primary-500 animate-in slide-in-from-bottom-2">
              ...
              <p className="text-gray-700 leading-relaxed mb-4 whitespace-pre-line">{notif.message}</p>
              <div className="flex items-center text-xs text-gray-400 border-t border-gray-100 pt-3">
                <span className="font-medium text-primary-600 mr-1">De :</span> {notif.authorName}
              </div>
            </div>
          ))
        ) : (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <p className="text-gray-500 mt-1">Célébrons ensemble nos réussites collectives.</p>
      </div>

      <div className="space-y-4">
        {victories.length > 0 ? (
          victories.map(victory => (
            <div key={victory.id} className="relative bg-gradient-to-br from-yellow-50 to-white p-6 rounded-xl shadow-sm border border-yellow-200 animate-in slide-in-from-bottom-2 overflow-hidden">
               <div className="absolute top-0 right-0 p-4 opacity-10">
                 <Trophy className="w-24 h-24 text-yellow-600" />
               </div>
               <div className="relative z-10">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-bold text-lg text-gray-900">{victory.title}</h3>
                    <span className="text-xs font-bold bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">
                      {victory.date}
                    </span>
                  </div>
                  <p className="text-gray-800 font-medium leading-relaxed">{victory.description}</p>
               </div>
            </div>
          ))
        ) : (

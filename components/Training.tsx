...
export const Training: React.FC<{currentUser: Member | null}> = ({currentUser}) => {
  ...
  if (!currentUser) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6 animate-in fade-in duration-500">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-3">Contenu Réservé aux Membres</h2>
        ...
        <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 max-w-sm w-full">
          <p className="text-sm font-medium text-gray-700">Déjà membre ?</p>
          <p className="text-xs text-gray-500 mt-1">Utilisez le bouton "Se connecter" dans le menu.</p>
        </div>
      </div>
    );
  }
  ...
  const getIconForType = (type: TrainingType) => {
    switch (type) {
      ...
      case 'LINK': return <LinkIcon className="w-6 h-6 text-gray-600" />;
        ...
        return (
          <div className="w-full h-full flex flex-col bg-white">
            <div className="bg-gray-50 p-3 border-b border-gray-200 flex justify-between items-center">
                <span className="text-sm text-gray-500 truncate max-w-2xl">{formattedUrl}</span>
                <a 
                  href={formattedUrl} 
                  target="_blank" 
                  rel="noopener noreferrer"
            ...
            <div className="flex-1 bg-gray-100 relative">
               ...
               <div className="absolute inset-0 -z-10 flex flex-col items-center justify-center p-10 text-center">
                  ...
                  <p className="text-xs text-gray-400 mt-2">Si rien ne s'affiche, le site bloque l'intégration. Utilisez le bouton ci-dessus.</p>
               </div>
            </div>
          </div>
        );
  ...
  return (
    <div className="space-y-6 pb-20 relative">
      ...
      {/* Header & Actions */}
      <div className="flex flex-col space-y-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Centre de Ressources</h2>
            <p className="text-sm text-gray-500">Formations, guides et outils pour votre développement</p>
          </div>
          {currentUser && (
            <button 
        ...
        {/* Filter Bar */}
        <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide border-t border-gray-100 pt-4">
            {filters.map(filter => (
              <button
                key={filter}
          ...
          filteredTrainings.map((resource) => (
            ...
            >
              <div className="p-5 flex-1">
                ...
                <div className="text-xs text-gray-400 flex justify-between items-center border-t border-gray-50 pt-3">
              ...
              <button 
                ...
                className="w-full bg-gray-50 p-3 text-sm font-medium text-gray-700 hover:bg-primary-50 hover:text-primary-700 transition-colors flex items-center justify-center border-t border-gray-100"
              >
                {resource.type === 'LINK' ? (
                  <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-xl font-bold text-gray-900">Ajouter une Formation</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-1">Titre de la formation</label>
                <input 
                  type="text" 
                  value={newTitle}
                  onChange={(e) => setNewTitle(e.target.value)}
                  className="w-full bg-white text-gray-900 border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none shadow-sm"
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-white text-gray-900 border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary-500 outline-none shadow-sm"
                  rows={3}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-900 mb-1">Type de contenu</label>
              ...
              <div>
                ...
                {newType === 'LINK' ? (
                  ...
                  <div 
                    className="border-2 border-dashed border-gray-300 bg-white rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 hover:border-primary-300 transition-colors"
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-400 mt-1">PDF, MP4, MP3, DOCX supportés</p>
                    <input 
                      type="file" 
                      ref={fileInputRef} 
                      className="hidden" 
      ...
      {viewingResource && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-white w-full h-full max-w-5xl max-h-[90vh] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-4 bg-white border-t border-gray-200 shrink-0">
              <p className="text-sm text-gray-600">{viewingResource.description}</p>
            </div>
          </div>
        </div>
      )}
